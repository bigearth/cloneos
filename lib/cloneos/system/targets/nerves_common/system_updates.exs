defmodule CloneOS.System.NervesCommon.Updates do
  defmacro __using__(target: _) do
    quote do
      @behaviour CloneOS.System.Updates
      require Logger
      ...
      defp blerp(tries) do
        ...
        if is_pid(pid) do
          r = CloneOS.Serial.Handler.write "F83"
          if r == :timeout do
      ...
      def post_install do
        ...
        :ok = blerp()
        r = CloneOS.Serial.Handler.write "F83"
        exp = Application.get_all_env(:cloneos)[:expected_fw_version]
        case r do
          ...
          other ->
            ...
            file = "#{:code.priv_dir(:cloneos)}/firmware.hex"
