defmodule CloneOS.System.NervesCommon.Network do
  @doc """
  ...
  defmacro __using__(target: _target, modules: modules) do
    quote do
      @behaviour CloneOS.System.Network
      use GenServer
      require Logger
      alias Nerves.InterimWiFi, as: NervesWifi
      alias CloneOS.System.Network.Hostapd
      ...
      def handle_info({Nerves.Udhcpc, :bound, %{ifname: interface, ipv4_address: ip}}, state) do
        if state.logging_in do
          ...
          that = self()
          spawn fn() ->
            CloneOS.System.Network.on_connect(fn() ->
      ...
      def handle_info({Nerves.WpaSupplicant, event, %{ifname: iface}}, state) when is_atom(event) do
        ...
        if wrong_key?, do: CloneOS.System.factory_reset
        if not_found?, do: CloneOS.System.factory_reset
        {:noreply, state}
      end
